...
class EyeToHandCalibrator:
    ...
    def capture_sample(self) -> bool:
        ...
        if self.save_dir:
            ...
            # tcp_rot(회전행렬) → ZYZ Euler 각도(도) 변환
            tcp_rx, tcp_ry, tcp_rz = self.robot._rotation_matrix_to_euler(tcp_rot)
            tcp_rx_deg = np.degrees(tcp_rx)
            tcp_ry_deg = np.degrees(tcp_ry)
            tcp_rz_deg = np.degrees(tcp_rz)
            # rvec → 마커 회전 (R_target2cam, Rodrigues 벡터)
            ...
            with open(log_path, 'a') as f:
                ...
                f.write(f"{sample_num},"
                        ...
                        f"{tcp_rx_deg:.4f},{tcp_ry_deg:.4f},{tcp_rz_deg:.4f},"
