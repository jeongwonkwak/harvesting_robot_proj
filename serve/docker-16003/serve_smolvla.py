...
async def lifespan(app: FastAPI):
    ...
    if not os.path.isabs(config.vlm_model_name):
        for candidate in [
            os.path.join(MODEL_PATH, config.vlm_model_name),
            os.path.join("/models/public", os.path.basename(config.vlm_model_name)),
        ]:
...
class PredictRequest(BaseModel):
    state: List[float]           # EEF pose: [x, y, z, roll, pitch, yaw]
...
class PredictResponse(BaseModel):
    action: List[float]          # EEF delta: [dx, dy, dz, droll, dpitch, dyaw, gripper]
    latency_ms: float
...
def predict(req: PredictRequest):
    ...
    if len(req.state) != 6:
        raise HTTPException(status_code=400, detail=f"state must be 6-dim EEF pose [x,y,z,roll,pitch,yaw], got {len(req.state)}")
