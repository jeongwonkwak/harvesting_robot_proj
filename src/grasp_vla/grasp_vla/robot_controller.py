...
class DoosanController:
    ...
    def __init__(self, node: Node, robot_id: str = "dsr01",
        ...
        # Subscribe to joint states (always available)
    ...
    def _init_services(self) -> None:
        ...
        self._pose_cli  = self._node.create_client(
            GetCurrentPose, f"{self._prefix}/aux_control/get_current_pose")
    ...
    def _joint_state_cb(self, msg: JointState) -> None:
        self._joint_state = np.array(msg.position, dtype=np.float64)
    ...
    def get_eef_pose(self) -> Optional[np.ndarray]:
        ...
        if fut.result():
            return np.array(fut.result().task_pos_info, dtype=np.float64)
        return None
    ...
    def move_joint(
        ...
        if self._sim:
            self._node.get_logger().info(
                f"[SIM] move_joint: {np.round(positions_deg, 2)}")
            return
        ...
        req.time      = 0.0
        req.radius    = 0.0
        req.ref       = 0    # joint space
        req.mode      = 0    # absolute
        req.blend_type = 0
        req.sync_type  = 1   # wait for completion
        fut = self._movej_cli.call_async(req)
        rclpy.spin_until_future_complete(self._node, fut, timeout_sec=30.0)
    ...
    def _execute_cartesian_delta(self, action: np.ndarray) -> None:
        ...
        delta_rot = np.clip(
            action[3:6] * _CART_ROT_SCALE,
            -_MAX_CART_DELTA_DEG,
             _MAX_CART_DELTA_DEG,
        )
        target = current + np.concatenate([delta_pos, delta_rot])
