...
from realsense_live import cv2_gui_available, sample_depth, setup_pipeline  # noqa: E402
...
def draw_fruit_box(ann: np.ndarray, x1: int, y1: int, x2: int, y2: int, name: str, conf: float) -> None:
    col = (0, 200, 0) if "ripe" in name else (0, 165, 255)
    cv2.rectangle(ann, (x1, y1), (x2, y2), col, 2)
    cv2.putText(ann, f"{name[:6]} {conf:.2f}", (x1, max(12, y1 - 4)),
...
def stem_on_fruit(
    ...
    *,
    ...
    for i in range(len(xyxy)):
        ...
        draw_fruit_box(ann, x1, y1, x2, y2, name, conf)
